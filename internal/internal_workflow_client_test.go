...
func (s *workflowClientTestSuite) TestPollActivityResultSurvivesMultipleIterations() {
	origDefaultRPC := defaultRPCTimeout
	origPollActivity := pollActivityTimeout
	defaultRPCTimeout = 100 * time.Millisecond
	pollActivityTimeout = 2 * time.Second
	defer func() {
		defaultRPCTimeout = origDefaultRPC
		pollActivityTimeout = origPollActivity
	}()
